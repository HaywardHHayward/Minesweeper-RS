...
impl Game {
    fn cell(&self, x: u8, y: u8) -> Element<'_, Action> {
        fn cell_view(cell: &crate::core::cell::Cell) -> Element<'_, Action> {
            ...
            if cell.is_open() {
                let open_image =
                    GuiImage::Image::new(GuiImage::Handle::from_bytes(image_data::OPENED_CELL));
                stack = stack.push(open_image);
                if cell.is_mine() {
                    let mine_image = cell_container(GuiImage::Image::new(
                        GuiImage::Handle::from_bytes(image_data::MINE),
                    ));
                ...
                let unopened_image =
                    GuiImage::Image::new(GuiImage::Handle::from_bytes(image_data::UNOPENED_CELL));
                stack = stack.push(unopened_image);
                if cell.is_flagged() {
                    let flag_image = cell_container(GuiImage::Image::new(
                        GuiImage::Handle::from_bytes(image_data::FLAG),
                    ));
